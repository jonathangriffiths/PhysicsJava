...
public class VectorMethods {
	...
	//corresponding to the elements (a,b,c) in each of two vectors.
	...
	//a 3d vector from the first three elements will be returned.
	...
	double angle (double v1a, double v1b, double v1c, double v2a, double v2b, double v2c){
		...
		double radians = Math.asin(dot_product/(magA*magB));
		double degrees = radians*180/Math.PI;
		return degrees;
	}

	//now we see when the array method becomes easier:
	//Function takes two arrays (must be one dimensional, contents as double, please.) Angle between
	// the vectors formed from the first three elements of each will be returned, in degrees.
	...
	public static void main(String[] args) {
		...
		System.out.println("");
		System.out.println("The angle between the vectors is (by element): "+vm.angle(v1a, v1b, v1c, v2a, v2b, v2c));
		System.out.println("The angle between the vectors is (by array): "+vm.angleArray(vector1, vector2));
		System.out.println("");
		...
		System.out.println("Both return NaN (not a number) because of 0/0. Here is 0/0: "+0.0/0.0+". Meanwhile, 1/0: "+1.0/0.0);
